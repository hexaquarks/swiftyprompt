...
describe("Codex connector", function()
    ...
    it("sends selection, prior turns, and the new question to Codex", function()
        ...
        assert.same("--output-last-message", command[6])
        assert.matches("local value = 1", command[8])
        assert.matches("Previous question: What is value%?", command[8])
        assert.matches("Question: What does this do%?", command[8])
